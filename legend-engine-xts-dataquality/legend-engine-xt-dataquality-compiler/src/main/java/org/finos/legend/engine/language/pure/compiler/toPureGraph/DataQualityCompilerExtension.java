...
public class DataQualityCompilerExtension implements CompilerExtension
{
    ...
    private LambdaFunction<?> buildDataqualityRelationValidationQuery(DataqualityRelationValidation dataqualityRelationValidation, CompileContext compileContext)
    {
        ...
        Assert.assertTrue(M2StorePaths.RelationStoreAccessor.equals(relationQueryReturnType), () -> "RelationStoreAccessor expected from lambda");
        return relationQuery;
    }

    private FastList<Root_meta_external_dataquality_RelationValidation> buildDataQualityRelationValidations(List<RelationValidation> relationValidations, LambdaFunction<?> relationQuery, SourceInformation sourceInformation, CompileContext compileContext)
    {
        FastList<Root_meta_external_dataquality_RelationValidation> root_meta_external_dataquality_relationValidations = FastList.newList();
        for (RelationValidation relationValidation : relationValidations)
        {
            root_meta_external_dataquality_relationValidations.add(buildRelationValidation(relationValidation, relationQuery, sourceInformation, compileContext));
        }
        return root_meta_external_dataquality_relationValidations;
    }

    private Root_meta_external_dataquality_RelationValidation_Impl buildRelationValidation(RelationValidation relationalValidation, LambdaFunction<?> relationQuery, SourceInformation sourceInformation, CompileContext compileContext)
    {
        Assert.assertTrue(relationalValidation.assertion.parameters.size() == 1, () -> "Assertion should have one input param!");
        Root_meta_external_dataquality_RelationValidation_Impl root_meta_external_dataquality_relationValidation_ = new Root_meta_external_dataquality_RelationValidation_Impl("", sourceInformation, compileContext.pureModel.getClass("meta::external::dataquality::RelationValidation"));
        root_meta_external_dataquality_relationValidation_._name(relationalValidation.name);
        root_meta_external_dataquality_relationValidation_._description(relationalValidation.description);
        ...
        assertionInputParam.relationType = RelationTypeHelper.convert(fetchQueryRelationType(relationQuery));

        LambdaFunction<?> assertion = HelperValueSpecificationBuilder.buildLambda(relationalValidation.assertion, compileContext);
        root_meta_external_dataquality_relationValidation_._assertion(assertion);
        return root_meta_external_dataquality_relationValidation_;
    }

    private RelationType<?> fetchQueryRelationType(LambdaFunction<?> relationQuery)
    {
        return (RelationType<?>) relationQuery._expressionSequence().getLast()._genericType()._typeArguments().getLast()._rawType();
    }
