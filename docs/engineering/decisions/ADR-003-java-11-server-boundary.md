...
| D-3 | Delete the 28 module-level `source`/`target=11` overrides rather than honour them | Class-file headers in `target/classes` showed all 28 emitting major 52 — the root `release=8` had been overriding them silently. Honouring them would have moved modules like `legend-engine-identity-core` (a `collection-generation` dependency) to Java 11 and broken the client contract. Deleting is a no-op on output and makes each pom truthful. |
