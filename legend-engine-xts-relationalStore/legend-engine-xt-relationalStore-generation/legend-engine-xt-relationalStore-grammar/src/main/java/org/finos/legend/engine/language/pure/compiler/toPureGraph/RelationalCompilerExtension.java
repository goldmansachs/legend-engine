...
public class RelationalCompilerExtension implements IRelationalCompilerExtension
{
    ...
    public Iterable<? extends Processor<?>> getExtraProcessors()
    {
        return Lists.immutable.with(
                Processor.newProcessor(
                        ...
                        (Database srcDatabase, CompileContext context) ->
                        {
                            if (!srcDatabase.includeGeneratedStores.isEmpty())
                            {
    ...
    private Set<PackageableElementPointer> databasePrerequisiteElementsPass(Database srcDatabase, CompileContext context)
    {
        return Stream.concat(
                        ...
                        srcDatabase.includeGeneratedStores.stream().filter(java.util.Objects::nonNull).map(p -> p.packageableElementPointer)
                )
